...
class Map:
    ...
    def decide_point(self, now_point_x, now_point_y, former_graph):  # 今のx,y座標 + 一個前の道の番号
        ...
        connection_road_numbers = []
        connection_road_numbers.append(connection_road)
    ...
    def moving_road(self, graph_number, now_point):
        ...
        if now_point == apex[0]:  # xが最大の時
            ...
            for i in range(self.roads_cost["graph" + str(graph_number).strip("[]")]):
                ...
                if apex_passed:
                    break
                plt.scatter(now_point_x, now_point_y, s=50, c="blue")
                plt.pause(.01)
                graph_now_point_x = now_point_x
                graph_now_point_y = now_point_y
                ...
                if random_number in [1, 2, 3, 4, 5]:  # 20分の１の確率で客が乗車

                    input_point_x = input("ここにx座標を入力してください:")
                    input_bool_x = True
                    if str.isdigit(input_point_x) == False:

                        while input_bool_x:
                            print("数字を入力してください")
                            input_point_x = input("ここにx座標を入力してください:")
                            if str.isdigit(input_point_x) == True:
                                input_bool_x = False

                    input_point_y = input("ここにy座標を入力してください:")
                    input_bool_y = True
                    if str.isdigit(input_point_y) == False:

                        while input_bool_y:
                            print("数字を入力してください")
                            input_point_y = input("ここにy座標を入力してください:")
                            if str.isdigit(input_point_y) == True:
                                input_bool_y = False
                    input_point = [int(input_point_x), int(input_point_y)]
                    input_bool = True
                    print(input_point)
                    select_points = []
                    for select_point in self.all_points:
                        select_points.append(select_point[0])
                    if input_point not in select_points:
                        ...
                        while input_bool:
                            ...
                            input_bool = True
                            input_point = [int(input_point_x), int(input_point_y)]
                            if input_point in select_points:
                                input_bool = False
                    input_bool = True
                    input_bool_point = True

                    for edge in self.edges:
                        if [now_point_x, now_point_y] in edge:
                            while input_bool_point:
                                if input_point == [now_point_x, now_point_y] or input_point == edge[0] or input_point == \
                                        edge[1]:
                                    print("現在地または、隣の座標は入力しないでください")
                                    while input_bool:
                                        ...
                                        if str.isdigit(input_point_y) == False:
                                            ...
                                            while input_bool_y:
                                                ...
                                                if str.isdigit(input_point_y) == True:
                                                    input_bool_y = False
                                        input_point = [input_point_x, input_point_y]
                                        print(input_point)
                                        if [int(input_point_x), int(input_point_y)] in select_points:
                                            input_bool = False
                                ...
                                for edge in self.edges:
                                    if [now_point_x, now_point_y] in edge:
                                        ...
                                        if input_point == [now_point_x, now_point_y] or input_point == edge[
                                            0] or input_point == edge[1]:

                                            break
                                        else:
                                            input_bool_point = False
                    ...
                    plt.scatter(graph_now_point_x, graph_now_point_y, s=50, c="green")
                    plt.pause(.01)
                    goal_point = input_point
                    price = 410  # priceを初期化
                    ...
                    passed.append(goal_point)

                    passed.pop(0)  # 経路から最初のNoneを取り除く

                    for graph_index, graph_pass_point in enumerate(passed):
                        if graph_index != len(passed)-1:
                            plt.plot([graph_pass_point[0], passed[graph_index + 1][0]], [graph_pass_point[1], passed[graph_index + 1][1]], "-k", c="red")
                            plt.pause(.01)

                    for road_index, pass_point in enumerate(passed):
                        plt.scatter(pass_point[0], pass_point[1], s=50, c="blue")
                        plt.pause(.01)
                        if apex_passed:
                            break
                        time.sleep(1)
                        plt.scatter(pass_point[0], pass_point[1], s=50, c="green")
                        plt.pause(.01)
                        if road_index != len(passed)-1:
                            plt.plot([pass_point[0], passed[road_index + 1][0]], [pass_point[1], passed[road_index + 1][1]], "-k")
                            plt.pause(.01)
            ...
            for i in range(self.roads_cost["graph" + str(graph_number).strip("[]")]):
                ...
                if random_number in [1, 2, 3, 4, 5]:  # 20分の１の確率で客が乗車
                    ...
                    for edge in self.edges:
                        if [now_point_x, now_point_y] in edge:
                            if input_point == [now_point_x, now_point_y] or input_point == edge[0] or input_point ==edge[1]:
                                print("現在地または、隣の座標は入力しないでください")
                                while input_bool:
                                    ...
                                    if str.isdigit(input_point_y) == False:
                                        ...
                                        while input_bool_y:
                                            ...
                                            if str.isdigit(input_point_y) == True:
                                                input_bool_y = False
                                    input_point = [input_point_x, input_point_y]
                                    if [int(input_point_x), int(input_point_y)] in self.all_points:
                    ...
                    for graph_index, graph_pass_point in enumerate(passed):
                        if graph_index != len(passed) - 1:
                            plt.plot([graph_pass_point[0], passed[graph_index + 1][0]],
                                     [graph_pass_point[1], passed[graph_index + 1][1]], "-k", c="red")
                            plt.pause(.01)
                    ...
                    for road_index, pass_point in enumerate(passed):
                        ...
                        plt.scatter(pass_point[0], pass_point[1], s=50, c="green")
                        if road_index != len(passed) - 1:
                            plt.plot([pass_point[0], passed[road_index + 1][0]],
                                     [pass_point[1], passed[road_index + 1][1]], "-k")
                            plt.pause(.01)
                        plt.pause(.01)
                        price += 60
                        print("お客さんを目的地まで運びます", pass_point[0], pass_point[1])
...
def move_map(start_point_x, start_point_y):
    ...
    print("startします", start_point_x, start_point_y)
